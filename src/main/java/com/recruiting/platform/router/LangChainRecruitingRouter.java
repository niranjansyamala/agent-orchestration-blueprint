...
public class LangChainRecruitingRouter implements RecruitingRouter {
    ...
    private RoutingDecision heuristicRoute(String query) {
        ...
        if ((normalized.contains("offer") || normalized.contains("compensation"))
            ...
            return new RoutingDecision(IntentType.REQUEST_OFFER_APPROVAL,
                    "offer_approval_agent",
                    0.92,
                    List.of("approval_workflow", "status_update"),
                    true);
        }
        if (normalized.contains("schedule") || normalized.contains("interview")) {
            return new RoutingDecision(IntentType.SCHEDULE_INTERVIEW,
                    "interview_scheduling_agent",
                    0.88,
                    List.of("calendar_lookup", "status_update"),
                    false);
        }
        if (normalized.contains("status") || normalized.contains("move candidate") || normalized.contains("advance candidate")) {
            return new RoutingDecision(IntentType.UPDATE_CANDIDATE_STATUS,
        ...
        if (normalized.contains("screen") || normalized.contains("resume") || normalized.contains("fit")) {
            return new RoutingDecision(IntentType.SCREEN_CANDIDATE,
                    "candidate_screening_agent",
                    0.89,
                    List.of("candidate_lookup", "job_lookup", "fit_summary"),
                    false);
        }
        if (normalized.contains("brief") || normalized.contains("summary")) {
            return new RoutingDecision(IntentType.GENERATE_RECRUITER_BRIEF,
                    "recruiter_copilot_agent",
                    0.85,
                    List.of("candidate_lookup", "job_lookup", "brief_generation"),
                    false);
        }
        return new RoutingDecision(IntentType.GENERAL_RECRUITING_SUPPORT,
                "recruiter_copilot_agent",
                0.70,
                List.of("brief_generation"),
                false);
    }
}
